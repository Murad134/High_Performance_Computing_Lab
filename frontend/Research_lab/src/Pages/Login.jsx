...
export default function Login() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // Authenticate with Firebase first. The backend document may be missing
      // after an interrupted registration, so it is synchronized below.
      setErrorMessage(""); // clear previous error message
      const result = await signIn(data.email, data.password);
      const user = result.user;
      ...
      // Re-create/synchronize the backend record if registration was interrupted.
